...
for (Mej, Eexp, MNi) in Mej_Eexp_MNi_array:
	...
	for (Mstar,t_TDE,visc_param) in itertools.product(Mstar_array, t_TDE_array, visc_param_array):
		...
		np.savetxt('Mej%g_Eej%s_Mstar%gMsun_tTDE%gday_visc%.1e.txt' % (int(Mej/con.Msun), str(Eexp).replace('+',''), Mstar/con.Msun, t_TDE/86400., visc_param), np.c_[t_arr/86400., L_arr, Lwind_arr, r_arr, v_arr/1e5, rneb_arr, Erad_arr, Egas_arr, epsrad_arr, tauej_arr, Teff_arr, Mwind_arr], header='time [day], luminosity [erg/s], L_wind [erg/s], r_ej [cm], v_ej [km/s], r_neb [cm], Erad [erg], Egas [erg], epsilon_rad, tau_ej, T_eff [K], M_wind [Msun]', fmt='%.6g')
